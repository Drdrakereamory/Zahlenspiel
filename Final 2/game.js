/* ============================================================
   ...
   Update: vollständige Tastatursteuerung ohne Maus
   ============================================================ */
...
function getHighscore(levelId) {
  return getHighscores()['lvl' + levelId] || null;
}
function updateHighscoreBanner() {
  ...
  const names = { lvl1:'01–99', lvl2:'100–999', lvl3:'1000–9999' };
  document.getElementById('highscore-text').textContent =
    'Bestmarke: ' + entries.map(([k,v]) => `${names[k]||k}: ${v} V.`).join(' · ');
...
// Keyboard state
let currentScreen   = 'start';   // 'start' | 'transition' | 'game' | 'end'
let currentMode     = 'normal';
let choiceFocusIdx  = 0;
...
document.addEventListener('keydown', (e) => {
  // Ziffern immer direkt weiterleiten wenn Spiel läuft
  if (currentScreen === 'game' && (currentMode === 'normal' || currentMode === 'brain')) {
    const inp = currentMode === 'normal'
      ? document.getElementById('main-input')
      : document.getElementById('brain-input');
    if (e.key >= '0' && e.key <= '9') {
      inp.focus();
      return; // Browser handelt das Input-Event
    }
    if (e.key === 'Backspace') {
      const val = typedValue.slice(0, -1);
      inp.value = val;
      if (currentMode === 'normal') handleDigitInput(val);
      else handleBrainInput(val);
      e.preventDefault();
      return;
    }
  }

  // Choice-Modus: Pfeiltasten + Enter
  if (currentScreen === 'game' && currentMode === 'choice') {
    const btns = [...document.querySelectorAll('.choice-btn:not(:disabled)')];
    if (!btns.length) return;
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
      choiceFocusIdx = (choiceFocusIdx + 1) % btns.length;
      btns[choiceFocusIdx].focus();
      e.preventDefault(); return;
    }
    if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
      choiceFocusIdx = (choiceFocusIdx - 1 + btns.length) % btns.length;
      btns[choiceFocusIdx].focus();
      e.preventDefault(); return;
    }
    if (e.key === 'Enter' || e.key === ' ') {
      btns[choiceFocusIdx]?.click();
      e.preventDefault(); return;
    }
    // Zahl direkt tippen = erste Übereinstimmung wählen
    if (e.key >= '0' && e.key <= '9') {
      const typed = e.key;
      const match = btns.find(b => b.textContent.startsWith(typed));
      if (match) { match.focus(); choiceFocusIdx = btns.indexOf(match); }
      return;
    }
  }

  // Slot-Modus: beliebige Taste = drehen
  if (currentScreen === 'game' && currentMode === 'slot') {
    const btn = document.getElementById('slot-btn');
    if (!btn.disabled && (e.key === 'Enter' || e.key === ' ')) {
      btn.click();
      e.preventDefault(); return;
    }
  }

  // Startscreen: Enter/Space = Spiel starten, Pfeiltasten = Level wählen
  if (currentScreen === 'start') {
    ...
    if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
      selectLevel(Math.min(3, selectedLevel + 1));
      e.preventDefault(); return;
    }
    if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
      selectLevel(Math.max(1, selectedLevel - 1));
      e.preventDefault(); return;
    }
  }

  // Übergangsscreen: beliebige Taste = weiter
  if (currentScreen === 'transition') {
    continueFromTransition();
    e.preventDefault(); return;
  }

  // Endscreen: beliebige Taste = nochmal
...
function nextAttempt() {
  ...
  if (attempt === 4 && !transitionShown) {
    ...
    // Hint anzeigen
    showKeyHint('transition', 'Beliebige Taste drücken um weiterzumachen');
    return;
  }
...
function setMode(mode) {
  ...
  if (mode === 'normal') {
    ...
    card.classList.add('phase-slot');
    setBadge('slot', lucideRefresh(), 'Slot Mode');
    setInstruction('<strong>Drücke Enter oder Leertaste</strong> um das Rad zu drehen — kein Mausklick nötig.');
    document.getElementById('slot-btn').focus();

  } else if (mode === 'choice') {
    document.getElementById('zone-choice').style.display = 'block';
    buildChoices();
    card.classList.add('phase-choice');
    setBadge('choice', lucideHelp(), '1 aus 10');
    setInstruction('<strong>Pfeiltasten</strong> zum Navigieren, <strong>Enter</strong> zum Wählen — oder direkt eine Ziffer tippen.');

  } else if (mode === 'brain') {
    document.getElementById('zone-brain').style.display = 'block';
    buildEquation();
    resetDigitDisplay('brain-digit-display');
    card.classList.add('phase-brain');
    setBadge('brain', lucideBrain(), 'Brain Mode');
    setInstruction('<strong>Löse die Gleichung</strong> und tippe das Ergebnis ein — automatische Auswertung.');
...
function showKeyHint(screen, text) {
  let el = document.getElementById('key-hint-' + screen);
  if (!el) {
    el = document.createElement('p');
    el.id = 'key-hint-' + screen;
    el.className = 'key-hint';
  }
  el.textContent = text;
  const card = document.querySelector('#screen-' + screen + ' .tr-card') ||
               document.querySelector('#screen-' + screen + ' .end-card');
...
function setupDigitSlots(containerId, digits) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (!el) continue;
    if (i < digits) {
      el.classList.remove('digit-hidden','filled','active','correct','wrong');
      el.textContent = '_';
    } else {
...
function handleDigitInput(val) {
  if (gameOver) return;
  const clean = String(val).replace(/\D/g, '').slice(0, currentDigits);
...
function processGuess(val) {
  const prefix = currentMode === 'brain' ? 'bd' : 'd';
...
function endGame() {
  ...
  document.getElementById('end-sub').textContent    = won
    ? `${secretNumber} in Versuch ${attempt} gefunden${isNew?' — neuer Rekord!':'.'}`
  ...
  showKeyHint('end', 'Beliebige Taste für eine neue Runde');
}
